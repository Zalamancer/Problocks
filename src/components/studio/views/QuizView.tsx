...
/**
 * Quiz Mode in the studio — developer-facing preview of the solo
 * quiz experiences (Drill + Homework). The teacher's live/self-paced
 * host dashboard lives at /teach/quiz (not here — the studio is for
 * developers, not classroom use). The student play screen lives at
 * /play/quiz/[pin].
 *
 * Why this file still exists: it lets someone iterating on the FRQ
 * content see the solo flows without spinning up a room.
 */
export function QuizView() {
  ...
  // Homework gets the full viewport (wide desktop layout).
  if (screen === 'homework') {
    return (
      <div style={fullBg}>
  ...
  return (
    ...
    >
      ...
      >
        <ModeSwitcher mode={mode} onPick={(m) => { setMode(m); setScreen('start'); }} />
...
function Footnote() {
  return (
    ...
    >
      ...
      The teacher dashboard lives at <code>/teach/quiz</code>; this is a solo preview for developers.
    </div>
  );
}
