...
export const metadata = {
  ...
  description: 'Every mutation performed via an admin surface.',
};

export default async function AuditLogPage() {
  const session = await getTeacherSession();
  if (!session?.user) {
    redirect('/classroom/auth?callbackUrl=/admin/audit-log');
  }
    ...
    <main style={page}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <header style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12, marginBottom: 22 }}>
          <div>
            <h1 style={{ fontSize: 28, fontWeight: 800, margin: 0, lineHeight: 1.1 }}>Audit log</h1>
            <p style={{ fontSize: 13.5, color: 'var(--pb-ink-muted, #6a6458)', marginTop: 4 }}>
              Every mutation performed via <code>/admin/*</code> surfaces,
              most recent first.
            </p>
          </div>
