...
export function DueDatePicker({ value, onChange, className }: DueDatePickerProps) {
  ...
  // ── Staged date (set when user picks a day, confirmed with time) ───────────
  ...
  // Label shown in the time-step header
      ...
      >
        ...
        {displayDate ? (
          <span className="flex-1 text-sm text-zinc-200 leading-none">
            {displayDate}
            {displayTime && (
              <span className="ml-2 text-zinc-500 text-xs">{displayTime}</span>
            )}
          </span>
        ) : (
        ...
        <div className={cn(
          ...
          {step === 'date' && (
            <div className="p-3">
              ...
              <div className="grid grid-cols-7 mb-1">
                {DAY_HEADERS.map(h => (
                  <div key={h} className="text-center text-[10px] font-medium text-zinc-600 py-1">
                    {h}
                  </div>
                ))}
              </div>
          ...
          {step === 'time' && (
            <div className="p-4">
              ...
              {/* Header: back chevron + selected date label */}
