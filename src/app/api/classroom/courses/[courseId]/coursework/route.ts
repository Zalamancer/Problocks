...
export async function GET(
  ...
  if (searchParams.get('courseWorkStates')) query.set('courseWorkStates', searchParams.get('courseWorkStates')!);
  if (searchParams.get('orderBy')) query.set('orderBy', searchParams.get('orderBy')!);

  const url = `${CLASSROOM_BASE}/courses/${courseId}/courseWork?${query}`;
